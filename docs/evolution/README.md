...
> **目的**：把 186 篇 paper notes 的"时间 × 派系 × 引用"3 维关系**可视化**，**d3 force-directed 2D**（早期 v1 试过 Three.js 3D，commit `db6e572` / `16f3e2a` 因 GitHub Pages ES module 不稳定回退到 r147 UMD；之后 commit `12966ff` 进一步简化为 2D 力导向布局）。
...
├── styles.css              # 样式
├── data/evolution.json     # 186 节点 + 56 边 (auto-gen)
└── README.md               # 本文件
