# Peking University

> **slug**: `PKU`  
> **所在地**: Beijing  
> **类型**: 高校  
> **Papers**: 7 篇 (paper-first 自动聚合)
> **合作价值**: **HIGH** — 7 papers + PI homepage/DBLP 可联系

## 代表老师

- **姓名**: Ziwei Liu（中文待补）
- **职称**: Professor (待人工确认职称)
- **个人主页**: https://liuziwei7.github.io/ (confidence: low)
- **团队规模**: 未知 (subagent 抓 publications HTML 失败 / 主线 HTML JS-rendered)
- **校友去向**: 未公开 (未抓到 alumni 数据 — 需后续 DBLP / 实验室主页核查)

## 代表作

### DreamGaussian4D: Generative 4D Gaussian Splatting
- **arXiv**: [2312.17142](https://arxiv.org/abs/2312.17142) (2023)
- **Faction**: A
- **Role**: NTU 主导；PKU (Ziwei Liu) 国内主通讯 (DreamGaussian4D)
- **Authors**: Jiawei Ren, Liang Pan, Jiaxiang Tang, Chi Zhang, Ang Cao, Gang Zeng

### 4D Gaussian Splatting with Scale-aware Residual Field and Adaptive Optimization for Real-time Rendering of Temporally Complex Dynamic Scenes
- **arXiv**: [2412.06299](https://arxiv.org/abs/2412.06299) (2024)
- **Faction**: A
- **Role**: PKU Shenzhen (4DGS scale-aware residual)
- **Authors**: Jinbo Yan, Rui Peng, Luyang Tang, Ronggang Wang

### 4D-Rotor Gaussian Splatting: Towards Efficient Novel View Synthesis for Dynamic Scenes
- **arXiv**: [2402.03307](https://arxiv.org/abs/2402.03307) (2024)
- **Faction**: A
- **Role**: PKU + Princeton + SKL of General AI (4D-RotorGS)
- **Authors**: Yuanxing Duan, Fangyin Wei, Qiyu Dai, Yuhang He, Wenzheng Chen, Baoquan Chen

### EMD: Explicit Motion Modeling for High-Quality Street Gaussian Splatting
- **arXiv**: [2411.15582](https://arxiv.org/abs/2411.15582) (2024)
- **Faction**: C
- **Role**: PKU + NIO + CAS (EMD street GS)
- **Authors**: Xiaobao Wei, Qingpo Wuwu, Zhongyu Zhao, Zhuangzhe Wu, Nan Huang, Ming Lu

### A Novel Benchmark and Dataset for Efficient 3D Gaussian Splatting with Gaussian Point Cloud Compression
- **arXiv**: [2505.18197](https://arxiv.org/abs/2505.18197) (2025)
- **Faction**: C
- **Role**: PKU Shenzhen + PCL (GausPCC compression)
- **Authors**: Kangli Wang, Shihao Li, Qianxi Yi, Wei Gao

### VicaSplat: A Single Run is All You Need for 3D Gaussian Splatting and Camera Estimation from Unposed Video Frames
- **arXiv**: [2503.10286](https://arxiv.org/abs/2503.10286) (2025)
- **Faction**: C
- **Role**: ZJU + Westlake (VicaSplat — first author dual-affil)
- **Authors**: Zhiqi Li, Chengrui Dong, Yiming Chen, Zhangchi Huang, Peidong Liu

### FieryGS: In-the-Wild Fire Synthesis with Physics-Integrated Gaussian Splatting
- **arXiv**: [2605.00177](https://arxiv.org/abs/2605.00177) (2026)
- **Faction**: C
- **Role**: PKU + ByteDance Seed (FieryGS)
- **Authors**: Qianfan Shen, Ningxiao Tao, Qiyu Dai, Tianle Chen, Minghan Qin, Yongjie Zhang

## 来源链接

- https://arxiv.org/abs/2312.17142
- https://arxiv.org/abs/2402.03307
- https://arxiv.org/abs/2411.15582
- https://arxiv.org/abs/2412.06299
- https://arxiv.org/abs/2503.10286
- https://arxiv.org/abs/2505.18197
- https://arxiv.org/abs/2605.00177
